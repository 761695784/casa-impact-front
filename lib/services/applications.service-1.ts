...
export const applicationsService = {
  ...
  /**
   * Récupère le blob d'un document joint SANS déclencher de téléchargement
   * navigateur — utilisé par la modale d'aperçu (PDF/image affichés en
   * ligne ; les autres formats retombent sur le bouton Télécharger, faute
   * de visionneuse intégrée pour .docx/.xlsx etc.)
   * Endpoint : GET /api/admin/applications/{id}/documents/{doc}/download
   * (même route que `downloadDocument`, simplement sans le `a.click()`.)
   */
  previewDocument: async (
    ...
    if (DATA_SOURCE === "mock") {
      const mockContent = `Aperçu non disponible en mode démonstration (candidature #${applicationId}, document ${documentKey}).`
      const blob = new Blob([mockContent], { type: "text/plain" })
      return { blob, mimeType: "text/plain", filename: `${documentKey}.txt` }
    }
    ...
    if (!res.ok) {
      throw new Error(`Impossible de charger l'aperçu du document (HTTP ${res.status})`)
    }
